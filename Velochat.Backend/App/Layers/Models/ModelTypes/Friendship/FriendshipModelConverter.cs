...
public static class FriendshipModelConverter
{
    ...
    /// Thrown when the InitiatorId, ReceiverId or Accepted of the friendship is null.
    /// </exception>
    public static CompleteFriendship ToCompleteModel(this Friendship friendship) => new()
    {
        InitiatorId = friendship.InitiatorId ?? throw new ModelNotCompleteException(),
        ReceiverId = friendship.ReceiverId ?? throw new ModelNotCompleteException(),
        Accepted = friendship.Accepted ?? throw new ModelNotCompleteException()
    };
    ...
    {
        InitiatorId = completeFriendship.InitiatorId,
        ReceiverId = completeFriendship.ReceiverId,
        Accepted = completeFriendship.Accepted
    };
}